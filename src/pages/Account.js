...
const Account = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const identurl = `https://prod-api.iogoods.de:4000/identity/${miner}`;
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (type === "transfersTo" || type === "transfersFrom") {
        array.push([
          ...
          { val: item.value / 1000000000000 + " IO" },
        ]);
      }
    }
    return array;
                    ...
                    {data.map((item, i) => (
                      <div className="one-graph-info" key={i}>
                        ...
                        <div className="graph-label">
                          {item.name}{" "}
                          <span className="block-span">{item.value} IO</span>
                        </div>
                      </div>
                    ))}
                    <div className="one-graph-info">
                      Total Balance {accoutInfo.free} IO
                    </div>
                  </div>
                </>
              )}
